...
public:
    ...
    Ins(uint32_t bits, InsFormat format) : ins_raw(bits), fmt(format) {
    }
    ...
    bool GetImm(int32_t* imm) const {
        ...
        switch (fmt) {
            ...
            case InsFormat::I:
                {
                    uint32_t res = (ins_raw & MASK_I_IMM_11_0 & (~MASK_MSB)) >> 20;
            ...
            case InsFormat::B:
                {
                    ...
                    uint32_t res = 0b0 | (imm_4_1 << 1) | (imm_10_5 << 5) | (imm_11 << 11);
                    uint32_t sign = ins_raw & MASK_MSB;
            ...
            case InsFormat::J:
                {
                    ...
                    uint32_t res = 0b0 | (imm_10_1 << 1) | (imm_11 << 11) | (imm_19_12 << 12);
                    uint32_t sign = ins_raw & MASK_MSB;
    ...
    static Ins MakeIns_R(uint32_t funct7, uint32_t rs2, uint32_t rs1, uint32_t funct3, uint32_t rd) {
        uint32_t ins = 0 ;
    ...
    static Ins MakeIns_I(int32_t imm, uint32_t rs1, uint32_t funct3, uint32_t rd, Ins_I_OPCODE opcode) {
    ...
    static Ins MakeIns_SLTIU(int32_t imm, uint32_t rs1, uint32_t rd) {
    ...
    static Ins MakeIns_S(int32_t imm, uint32_t rs2, uint32_t rs1, uint32_t funct3) {
        int8_t sign = (imm < 0) ? 1 : 0;
        imm = abs(imm);
    ...
    static Ins MakeIns_B(int32_t imm, uint32_t rs2, uint32_t rs1, uint32_t funct3) {
        int8_t sign = (imm < 0) ? 1 : 0;
        imm = abs(imm);
    ...
    static Ins MakeIns_J(int32_t imm, uint32_t rd) {
        int8_t sign = (imm < 0) ? 1 : 0;
        imm = abs(imm);
...
private:
    ...
    static inline uint32_t InsSetValueMask(uint32_t ins, uint32_t val, uint32_t mask, size_t offset) {
        ins &= (~mask);
        val = (val << offset) & mask;
        return ins | val;
    }
